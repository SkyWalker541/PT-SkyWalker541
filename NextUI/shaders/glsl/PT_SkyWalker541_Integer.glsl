/*
╔══════════════════════════════════════════════════════════════════╗
║  PT SkyWalker541 Integer  v1.5.0                                 ║
...
╚══════════════════════════════════════════════════════════════════╝
  ...
  Use this variant for NATIVE / INTEGER scaling.
  Use PT_SkyWalker541_Aspect.glsl for ASPECT RATIO / NON-INTEGER scaling.
...
╚══════════════════════════════════════════════════════════════════╝
  ...
  v1.3.0 - Added PT_SYSTEM = 4 (GBA Original, threshold 0.38) tuned
           ...
           removing half-texel border misalignment and asymmetric border
           edges in the distFromCenter method at integer scale.
  ...
  v1.1.4 - Version bump for parity with Aspect variant (no changes).

  v1.1.3 - Fixed shadow performance — removed redundant
           floor()/TextureSize re-snapping from blur taps.

  v1.1.2 - Fixed drop shadows — removed redundant isWhitePixel gate
           that blocked shadows inside large white fills.

  v1.1.1 - Fixed drop shadows — taps now snap to texel centre.

  v1.1.0 - Split into Aspect and Integer variants. Integer uses
           distFromCenter pixel border (clean at integer scale).
           Shadow formula rewritten using TextureSize/InputSize/
           OutputSize only — drops OrigInputSize (unreliable on NextUI).
...
#pragma parameter PT_SYSTEM "== PT SkyWalker541 Integer v1.5.0 == System (0=Manual, 1=GB, 2=GBC, 3=GBA SP, 4=GBA Orig)" 1.0 0.0 4.0 1.0
...
// │  Pixel Border                    │
// └──────────────────────────────────┘
// distFromCenter method — produces a clean, sharp grid at integer/native scale.
// At native integer scale the grid is particularly crisp — consider mode 2 or 3.
...
// Integer pixel border constants (zfast_lcd distFromCenter method)
...
// distFromCenter method (zfast_lcd equation: y = a * (x^4 - b * x^6)).
// Uses distance from the center of each logical pixel in texture space.
// Produces a clean, sharp grid at integer/native scaling.
float pixelBorderFactor(vec2 coord)
{
    if (PT_PIXEL_BORDER < 0.5) return 1.0;

    vec2 imgPixelCoord  = coord * TextureSize;
    vec2 imgCenterCoord = floor(imgPixelCoord) + vec2(0.5);
    vec2 distFromCenter = abs(imgCenterCoord - imgPixelCoord);

    // Use the larger of x/y distance for a square grid pattern
    float x        = max(distFromCenter.x, distFromCenter.y);
    float xSquared = x * x;
    ...
    if (PT_PIXEL_BORDER < 1.5) {
        width = 0.55; strength = 0.40; // Subtle
    } else if (PT_PIXEL_BORDER < 2.5) {
        width = 0.75; strength = 0.65; // Moderate
    } else {
        width = 0.95; strength = 0.85; // Strong
    }

    lineWeight = lineWeight * (lineWeight + ((1.0 - lineWeight) * width));
