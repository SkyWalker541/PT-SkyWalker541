...
This version runs on standard RetroArch using a single-pass `.glslp` preset. White detection runs against the raw unprocessed game frame via RetroArch's built-in `OrigTexture` uniform, giving cleaner and more accurate results than would otherwise be possible.
...
- `OrigTexture` — the raw pre-correction input frame, used exclusively for white pixel detection and transparency gating
...
Requires a RetroArch video driver that supports GLSL shaders. This includes the **gl** and **glcore** drivers. Does not work with the **vulkan** driver — use `PT_SkyWalker541.slangp` for Vulkan.
...
| Version | Notes |
|---|---|
| v1.4.0 | Rebuilt as single-pass shader. Passthrough pass removed. `PassPrev2Texture` replaced with RetroArch's built-in `OrigTexture` uniform, which provides the raw pre-correction input frame natively without a passthrough pass. `orig_coord` computed in vertex shader for correct sampling. Loops unrolled for GLSL ES compatibility. Resolves missing shader and load failures on Android. All shader logic, parameters, and visual output identical to v1.3.0 |
| v1.5.0 | Added PT_SYSTEM = 4 (GBA Original) with threshold 0.75, tuned for the original GBA's dim, creamy whites. Reduced procedural background noise from three octaves to two — lower arithmetic cost with no visible quality difference. Fixed undefined `OrigTextureSize`/`OrigInputSize` uniforms in vertex and fragment stages. Fixed `orig_coord` vertex calculation. Fixed shadow offset formula. Fixed `getBrightness` for GLSL ES compatibility |
